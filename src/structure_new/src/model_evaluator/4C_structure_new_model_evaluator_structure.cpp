...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void Solid::ModelEvaluator::Structure::output_rank_eval_times() const
{
  ...
  output_data["RankEvalTime"] = discret().get_rank_eval_times();
  const double time = global_state().get_time_np();
  const int step = global_state().get_step_np();
  runtime_csvwriter_rank_eval_times_->write_data_to_file(time, step, output_data);
}
