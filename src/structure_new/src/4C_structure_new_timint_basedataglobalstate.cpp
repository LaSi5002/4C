    ...
    const Core::Rebalance::RebalanceParameters& rebalance_parameters)
{
  ...
  const auto remap_and_restore_mstep = [&remap_mstep, &overwrite_nodal_state](
                                           TimeStepping::TimIntMStep<Core::LinAlg::Vector<double>>&
                                               state,
                                           const std::vector<
  ...
  const auto remap_and_restore_vector = [&remap_vector, &overwrite_nodal_state](
                                             std::shared_ptr<Core::LinAlg::Vector<double>>&
                                                 vector_ptr,
                                             const std::shared_ptr<Core::LinAlg::MultiVector<double>>&
                                                 old_nodal_state)
  {
  ...
  for (auto& preserved_mstep_state : preserved_mstep_states)
    remap_and_restore_mstep(*preserved_mstep_state.state,
        preserved_mstep_state.nodal_state_per_step);

  for (auto& preserved_vector_state : preserved_vector_states)
    remap_and_restore_vector(
        *preserved_vector_state.state, preserved_vector_state.nodal_state);
