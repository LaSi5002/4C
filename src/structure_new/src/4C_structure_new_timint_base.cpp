...
bool Solid::TimeInt::Base::perform_dynamic_rebalance()
{
  ...
  parameters.mesh_partitioning_parameters.imbalance_tol =
      rebalance_params.get<double>("IMBALANCE_TOL");
  parameters.mesh_partitioning_parameters.min_ele_per_proc =
      rebalance_params.get<int>("MIN_ELE_PER_PROC");
  // The timing-weighted PHG path is still unstable on redistributed node graphs on this branch.
  // Keep the trigger timing-based, but use the proven static-cost partitioning path so dynamic
  // redistribution itself can run end-to-end first.
